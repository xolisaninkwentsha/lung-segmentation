...
def masked(img, mask, alpha=1):
    ...
    mask = morphology.dilation(mask, morphology.disk(3)) ^ mask
...
if __name__ == '__main__':
    ...
    csv_path = '/home/xolisaninkwentsha/Desktop/chest_xray/test_normal.csv'
    ...
    for xx in test_gen.flow(X, batch_size=1):
        ...
        io.imsave('results/{}'.format(df.iloc[i][0].split('/')[-1]), masked(img, pr, 1))
        # io.imsave('results/img{}'.format(df.iloc[i][0]), img)
        # io.imsave('results/gt{}'.format(df.iloc[i][0]), gt)
        # io.imsave('results/pr{}'.format(df.iloc[i][0]), pr)
        io.imsave('results/new{}'.format(df.iloc[i][0].split('/')[-1]), pr*img)
        # ious[i] = IoU(gt, pr)
        # dices[i] = Dice(gt, pr)
